...
{{< figure src="/images/202108/kubernetes-debug/readniess-probe.png" title="Readiness Probe Ready" height="400" width="400">}}

当这个 pod 指针判断一直处在失败状态的时候，其实接入层的流量不会打到现在这个 pod 上。

{{< figure src="/images/202108/kubernetes-debug/readniess-probe-1.png" title="Readiness Probe Fail" height="400" width="400">}}
...
{{< figure src="/images/202108/kubernetes-debug/liveness-probe.png" title="Liveness Probe Fail" height="400" width="400">}}
...
首先先看一下 exec，exec 的使用其实非常简单。如下YAML所示，大家可以看到这是一个 Liveness probe，它里面配置了一个 exec 的一个诊断。接下来，它又配置了一个 command 的字段，这个 command 字段里面通过 cat 一个具体的文件来判断当前 Liveness probe 的状态，当这个文件里面返回的结果是 0 时，或者说这个命令返回是 0 时，它会认为此时这个 pod 是处在健康的一个状态。
```yaml
apiVersion: v1
kind: Pod
metadata:
...
![请添加图片描述](https://img-blog.csdnimg.cn/b310c7a8bc314944846a4e1c0900560f.png?x-oss-process=image/watermark,type_ZmFuZ3poZW5naGVpdGk,shadow_10,text_aHR0cHM6Ly9ibG9nLmNzZG4ubmV0L3dlaXhpbl80MzM0MDk0Mw==,size_16,color_FFFFFF,t_70)
**介绍** 
...
一个 pod 里面有多个 container，每个 container 上面又会有一个字段叫 State，然后 State 的状态表示当前这个 container 的一个聚合状态。
...
首先先将 Telepresence 的一个 Proxy 应用部署到远程的 K8s 集群里面。然后将远程单一个 deployment swap 到本地的一个 application，使用的命令就是 `Telepresence-swap-deployment $DEPLOYMENT_NAME` 然后以及远程的 DEPLOYMENT_NAME。通过这种方式就可以将本地一个 application 代理到远程的 service 之上、可以将应用在远程集群里面进行本地调试.
